...
def build_and_push_docker_image(provider_json_file, ecr_repo, docker_file, docker_file_dir, log_file):
    ...
    auth_config_payload = get_docker_push_aws_auth_config(aws_access_key, aws_secret_key, aws_session_token, region_name, log_file)
    write_to_debug_log(log_file, "auth_config: " + str(auth_config_payload))
    docker_client = build_docker_image(docker_file_dir, docker_file, ecr_repo, log_file)
    latest_tag = "latest"
    pushed = docker_client.images.push(ecr_repo, tag=latest_tag, auth_config=auth_config_payload)#, stream=True, decode=True)
...
def delete_docker_images_from_local(docker_file_abs_path):
    ...
    docker_client.images.remove(ecr_repo, force=True)
    ...
    try:
        ...
        docker_client.images.remove(local_image_name, force=True)
