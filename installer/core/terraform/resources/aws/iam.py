...
class IAMRoleResource(TerraformResource):
    ...
    def check_exists_before(self, input, tf_outputs):
        ...
        if not self.resource_in_tf_output(tf_outputs):
            exists = iam.check_role_exists(checked_details['value'], input.aws_access_key, input.aws_secret_key, input.aws_session_token)
...
class IAMRolePolicyResource(TerraformResource):
    ...
    def check_exists_before(self, input, tf_outputs):
        ...
        exists = False
        if not self.resource_in_tf_output(tf_outputs):
            exists = iam.check_policy_exists(checked_details['value'], input.aws_access_key, input.aws_secret_key, input.aws_session_token, input.aws_account_id)
...
class IAMInstanceProfileResource(TerraformResource):
    ...
    def check_exists_before(self, input, tf_outputs):
        ...
        exists = False
        if not self.resource_in_tf_output(tf_outputs):
            exists = iam.check_instance_profile_exists(checked_details['value'], input.aws_access_key, input.aws_secret_key, input.aws_session_token)
